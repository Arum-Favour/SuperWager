...
import { updatePoolParticipation, getPoolParticipants } from '@/lib/firebase/firestore';

const POOL_CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POOL_CONTRACT_ADDRESS || "0x1234567890123456789012345678901234567890";

// Type for ethers v5 provider
type EthersProvider = ethers.providers.Web3Provider | ethers.providers.JsonRpcProvider;

export const getPoolStatus = async (provider: EthersProvider) => {
  try {
    // Mock data for development when contract is not available
    ...
    const poolContract = new ethers.Contract(POOL_CONTRACT_ADDRESS, poolContractABI.abi, provider);
    
    // Note: Adjust these method names based on your actual contract
    let playerCount, poolBalance;
    
    try {
      [playerCount, poolBalance] = await Promise.all([
        poolContract.getPlayerCount ? poolContract.getPlayerCount() : Promise.resolve(ethers.BigNumber.from(0)),
        poolContract.getPoolBalance ? poolContract.getPoolBalance() : Promise.resolve(ethers.BigNumber.from(0))
      ]);
    } catch (contractError) {
...
export const enterPool = async (signer: ethers.Signer, walletAddress: string) => {
  try {
    if (!process.env.NEXT_PUBLIC_POOL_CONTRACT_ADDRESS) {
    ...
    const poolContract = new ethers.Contract(POOL_CONTRACT_ADDRESS, poolContractABI.abi, signer);
...
export const closePoolAndDistribute = async (signer: ethers.Signer) => {
  try {
    if (!process.env.NEXT_PUBLIC_POOL_CONTRACT_ADDRESS) {
      ...
      return {
        closeTx: "0x1111111111111111111111111111111111111111111111111111111111111111",
        selectTx: "0x2222222222222222222222222222222222222222222222222222222222222222", 
        distributeTx: "0x3333333333333333333333333333333333333333333333333333333333333333",
        winners: participants.slice(0, Math.ceil(participants.length * 0.1)).map(p => p.walletAddress)
      };
    }
