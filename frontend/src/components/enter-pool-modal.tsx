  ...
  const handleEnterPool = async () => {
    ...
    try {
      ...
      if (userWalletAddress) {
        ...
        // Store the betting slip data in Firebase
        await storeBettingSlip(userWalletAddress, {
          poolId: poolId ?? "",
          slips,
          poolAmount: poolOption,
          entryTime: new Date(),
        });
        
        console.log(`✅ User ${userWalletAddress} entered pool and slip data stored in Firebase`);
        console.log(`📊 Slip details:`, {
          poolId,
          slipsCount: slips.length,
          totalOdds: slips.reduce((acc, slip) => acc + parseFloat(slip.odds), 0),
          poolAmount: poolOption,
        });
      } else {
        console.warn("⚠️ Could not get wallet address for Firebase sync");
      ...
      setHasEnteredPool(true);
      toast.success("You have entered the pool and your slip has been saved!");
      close();
      localStorage.setItem(
        "game",
        JSON.stringify({
      ...
      <div className="bg-white rounded-xl md:rounded-[20px] px-4 sm:px-6 md:px-10 py-10 relative flex flex-col items-center justify-center gap-6 md:gap-10 max-w-3xl">
        ...
        <div className="flex w-full flex-col gap-4 md:gap-6">
          <div className="flex items-center justify-center gap-4 p-1 px-4 border border-red-400 rounded-full">
            <TriangleAlert color="red" className="size-4 md:size-6" />
            <p className="text-sm md:text-xl font-medium sm:font-semibold">
              Once you enter pool, your slip will be saved and cannot be edited.
            </p>
          </div>
          <div className="flex justify-between items-center gap-4">
            <p className="text-sm sm:text-xl md:text-2xl font-medium">
          ...
          {/* Add slip summary */}
          <div className="flex justify-between items-center gap-4 bg-gray-50 p-3 rounded-lg">
            <div className="text-sm">
              <p className="font-medium">Your Slip Summary:</p>
              <p className="text-gray-600">{slips.length} matches selected</p>
              <p className="text-gray-600">
                Total odds: {slips.reduce((acc, slip) => acc + parseFloat(slip.odds), 0).toFixed(2)}
              </p>
            </div>
          </div>
